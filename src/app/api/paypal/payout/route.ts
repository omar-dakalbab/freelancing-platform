...
export async function POST(req: Request) {
  try {
    ...
    if (!profile.paypalEmail) {
      return NextResponse.json(
        { error: { code: "NOT_ONBOARDED", message: "Please add your PayPal email in payout settings before requesting a payout" } },
        { status: 422 }
      );
    }
    ...
    try {
      const result = await createPayPalPayout({
        ...
        amount: netAmount,
        note: `Payout for: ${payment.contract.job.title}`,
      ...
      await prisma.payout.update({
        ...
        data: {
          status: "FAILED",
          failureReason: paypalError instanceof Error ? paypalError.message : "PayPal payout failed",
        },
      });

      console.error("[POST /api/paypal/payout] Payout failed:", paypalError);
      return NextResponse.json(
        { error: { code: "TRANSFER_FAILED", message: "PayPal payout failed. Please try again later." } },
        { status: 500 }
      );
    }
  } catch (error) {
    console.error("[POST /api/paypal/payout]", error);
    return NextResponse.json(
